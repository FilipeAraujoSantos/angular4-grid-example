...
export class ListagemComponent implements OnInit {
  ...
  filtar(){
    ...
    let datasFiltradas = this.data.filter((casodeuso, indice) => {
        ...
        Object.values(casodeuso).forEach(valor => {
  ...
  alteraIconeOrdenacaoCSS(){
    ...
    let elementos:any = document.querySelectorAll('thead>tr>th>a>.glyphicon');
    
    if(this.classificacao == 'asc')
        elementos.forEach(span => span.classList.replace('glyphicon-sort-by-alphabet-alt','glyphicon-sort-by-alphabet'));    
    else if(this.classificacao == 'desc')
        elementos.forEach(span => span.classList.replace('glyphicon-sort-by-alphabet','glyphicon-sort-by-alphabet-alt'));
  ...
  getArrayOfAtributos(registro):string[]{
    ...
    if(registro){
        Object.entries(registro).forEach(atributo => {
